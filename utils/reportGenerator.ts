...
export const generateExcelReport = (
    ...
    for (const id of ids) {
        ...
        if (sheetRows && sheetRows.length > 0) {
            ...
            const authCol = Object.keys(sheetRows[0]).find(k => k.startsWith("Authorization"));
            if (authCol) columnsToCheck.push(authCol);
            
            const payloadCol = Object.keys(sheetRows[0]).find(k => k.startsWith("Request Payload"));
            if (payloadCol) columnsToCheck.push(payloadCol);

            columnsToCheck.forEach(col => {
                const isEmpty = sheetRows.every(row => !row[col] || String(row[col]).trim() === "");
                if (isEmpty) {
                    sheetRows.forEach(row => delete row[col]);
                }
            });

            const worksheet = XLSX.utils.json_to_sheet(sheetRows, { origin: "A4" });
            ...
            const wscols = Object.keys(sheetRows[0]).map(key => ({ wch: colWidths[key] || 20 }));
                ...
                for (let C = range.s.c; C <= range.e.c; ++C) {
                    ...
                    if (R === 0 || R === 1) {
                        ...
                        if (R > 3) {
                            ...
                            if (worksheet[headerCell] && worksheet[headerCell].v === "Status") {
                                if (worksheet[cellAddress].v === "Pass") {
                                    worksheet[cellAddress].s = { ...dataStyle, font: { color: { rgb: "10B981" }, bold: true } };
                                } else if (worksheet[cellAddress].v === "Fail") {
                                    worksheet[cellAddress].s = { ...dataStyle, font: { color: { rgb: "EF4444" }, bold: true } };
                                }
                            }
                        }
                    }
    ...
    if (hasData) {
        XLSX.writeFile(workbook, `${project.name.replace(/\s+/g, '_')}_Execution_Report.xlsx`);
    }
};
